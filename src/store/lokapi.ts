...
export function lokapiStoreFactory(lokApiService: any, passwordUtils: any) {
  return {
        ...
        try {
          ...
          if (!(err instanceof LokapiExc.BackendUnavailableTransient)) {
            if (state.accountsLoadingErrors.every((e: any) => e !== err)) {
              commit("setAccountsLoadingErrors", [err, ...state.accountsLoadingErrors])
              console.error("Exception while fetching account validation rights", err)
            }
          }
          hasRight = false
        }
      ...
      async fetchCreditRequestValidationRights({ commit, state }: any) {
        ...
        try {
          ...
          if (!(err instanceof LokapiExc.BackendUnavailableTransient)) {
            if (state.accountsLoadingErrors.every((e: any) => e !== err)) {
              commit("setAccountsLoadingErrors", [err, ...state.accountsLoadingErrors])
              console.error("Exception while fetching credits validation rights", err)
            }
          }
          hasRight = false
        }
    ...
    getters: {
      ...
      unavailableVirtualAccounts: (state: any) => {
        return state.virtualAccountTree.filter(
          (a: any) => a instanceof Array && a.every((e) => e instanceof Error)
        )
      },
